  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setTopics(topicsResponse.data.results.slice(0, 3)); // Show only recent 3
        setSessions(sessionsResponse.data.results.slice(0, 3)); // Show only recent 3
      } catch (error) {
            ...
            <CardContent>
              {sessions.length > 0 ? (
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-secondary-500">No upcoming sessions</p>
                  {user?.role === 'MODERATOR' && (
